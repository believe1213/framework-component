...
/**
 * 重写 {@link org.springframework.kafka.annotation.KafkaListenerAnnotationBeanPostProcessor}
 *
...
public class KafkaListenerAnnotationBeanPostProcessor<K, V>
    ...
    public Object postProcessAfterInitialization(final Object bean, final String beanName) throws BeansException {
        ...
        // -------------------- 源码修改start --------------------
        // 获取messageHandlerMethodFactory的引用
        KafkaListenerContainerUtil.messageHandlerMethodFactory = this.messageHandlerMethodFactory;
        // -------------------- 源码修改end --------------------
        return bean;
    }
    ...
    protected void processListener(MethodKafkaListenerEndpoint<?, ?> endpoint, KafkaListener kafkaListener,
        ...
        // -------------------- 源码修改start --------------------
        // 缓存kafkaListener注解配置信息
        ApplicationConstant configuration = this.beanFactory.getBean(ApplicationConstant.class);
        String env = configuration.getEnv();
        DebugTypeEnum typeEnum = GrayDebugUtils.getDebugType(env);
        // 所有灰度节点（含开发人员本地服务）、以及STG、UAT环境基准节点需要缓存kafka监听注解信息
        if (typeEnum == DebugTypeEnum.MULTI_VERSION || GrayDebugUtils.isGrayHost()) {
            String[] topics = resolveTopics(kafkaListener);
            if (ArrayUtils.isNotEmpty(topics)) {
                logger.info("缓存@kafkaListener注解配置信息，topic：" + FilterJsonUtils.deserializer(topics));
                if (Objects.isNull(factory)) {
                    factory = this.beanFactory.getBean(this.defaultContainerFactoryBeanName, KafkaListenerContainerFactory.class);
                }
                for (String topic : topics) {
                    KafkaListenerContainerUtil.KAFKA_LISTENER_ENDPOINT_MAP.put(topic, endpoint);
                    KafkaListenerContainerUtil.KAFKA_LISTENER_CONTAINER_FACTORY_MAP.put(topic, factory);
                }
            }
        }
        if (factory instanceof AbstractKafkaListenerContainerFactory) {
            logger.info("服务启动时设置KafkaListenerContainerFactory为：false");
            ((AbstractKafkaListenerContainerFactory<?, ?, ?>) factory).setAutoStartup(false);
        }
        // -------------------- 源码修改end --------------------
