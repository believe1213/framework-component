...
/**
 * 重写 ${@link org.apache.kafka.clients.producer.ProducerConfig}
 *
...
public class ProducerConfig extends AbstractConfig {
    ...
    private static String producerInterceptor = "com.minister.framework.debug.kafka.interceptor.DebugProducerInterceptor";
    ...
    public static Map<String, Object> buildConfig(Map<String, Object> props) {
        boolean b = false;
        try {
            Class onwClass = Class.forName(producerInterceptor);
            if (onwClass != null) {
                b = true;
            }
        } catch (Exception e) {
        ...
        //加入拦截
        Object object = props.get(ProducerConfig.INTERCEPTOR_CLASSES_CONFIG);
        if (object == null) {
            ArrayList<String> cls = new ArrayList<>();
            cls.add(producerInterceptor);
            props.put(ProducerConfig.INTERCEPTOR_CLASSES_CONFIG, cls);
        } else if (object instanceof List) {
            List<String> cls = (List<String>) object;
            if (!cls.contains(producerInterceptor)) {
                cls.add(producerInterceptor);
            }
        } else {
            ArrayList<String> cls = new ArrayList<>();
            if (!object.toString().equals(producerInterceptor)) {
                cls.add(producerInterceptor);
            } else {
