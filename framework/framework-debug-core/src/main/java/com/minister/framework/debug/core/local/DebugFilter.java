package com.minister.framework.debug.core.local;
...
public class DebugFilter extends GenericFilterBean {
    ...
    public void doFilter(ServletRequest sRequest, ServletResponse servletResponse, FilterChain chain) throws IOException, ServletException {
        try {
            ...
            if (FilterUtils.skipPathUri(uri) || DebugForwardHelper.isCacheEmpty()) {
            ...
            if (DebugConstants.POST.equalsIgnoreCase(hsRequest.getMethod()) &&
                ...
                String host = debugForwardHelper.checkForwardHost(uri, body);
            ...
            if (DebugConstants.GET.equalsIgnoreCase(hsRequest.getMethod())) {
                ...
                String host = debugForwardHelper.checkForwardHost(uri, paramJson(queryString));
                if (StringUtils.isNotBlank(host)) {
            ...
            logger.error("DebugFilter request/response error!");
            throw e;
        }
    }
    ...
    private void forwardFormUrlencoded(HttpServletRequest hsRequest, ServletResponse servletResponse, String host, String uri) throws IOException {
        try {
            ...
            logger.info("online debug>>>>>forward host : " + rHost);
            List<Header> list = debugForwardHelper.getOriginalHeaders(hsRequest);
            ...
            CloseableHttpResponse forwardResponse = debugForwardHelper.execute(DebugConstants.HTTP_PREFIX + rHost, uri, hsRequest.getMethod(), new UrlEncodedFormEntity(items), headers);
            debugForwardHelper.setForwardResponse((HttpServletResponse) servletResponse, forwardResponse);
        } finally {
            clearContext();
        }
    }
    ...
    private void forward(HttpServletRequest hsRequest, ServletResponse servletResponse, String host, String uri) throws IOException {
        try {
            ...
            CloseableHttpResponse forwardResponse = debugForwardHelper.execute(DebugConstants.HTTP_PREFIX + rHost, uri, method, entity, headers);
            debugForwardHelper.setForwardResponse((HttpServletResponse) servletResponse, forwardResponse);
        } finally {
            clearContext();
        }
